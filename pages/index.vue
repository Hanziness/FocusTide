  ...
  <div class="h-screen overflow-x-hidden overflow-y-scroll">
      ...
      <Columns class="flex-col gap-8 px-4 pt-8 xl:flex-row xl:space-x-16 xl:px-24 xl:pt-0">
        ...
        <template #right>
          <div :class="['transition-all duration-1000 hover:duration-300 overflow-hidden order-first xl:order-last rounded-lg shadow-red-300/60 shadow-lg hover:-translate-y-1 hover:shadow-xl hover:shadow-red-300/70 w-full max-w-2xl xl:max-w-3xl self-center', { 'opacity-0 translate-x-4': !loading.screenshot }]">
            <img
              :alt="$t('index.alt.img.screenshot')"
              class=""
              width="1600"
              height="900"
      ...
      <template #after>
        <div class="flex flex-col items-center justify-end flex-grow mb-8 xl:absolute xl:bottom-4">
          ...
          <div class="flex flex-row mb-4 space-x-2">
            <SupportButton
              :aria-label="$t('index.alt.links.source')"
              icon-size="28"
              type="github"
              :show-text="false"
              :default-colours="false"
              class="p-2 text-gray-100 bg-black hover:bg-gray-800 active:bg-gray-900"
              utm-tags="?utm_source=AnotherPomodoro&utm_medium=web&utm_content=home"
              tabindex="0"
            />
            <SupportButton
              :aria-label="$t('index.alt.links.support')"
              icon-size="28"
              type="support"
              :show-text="false"
              :default-colours="false"
    ...
    <Section ref="section-2" class="flex flex-col justify-center px-4 overflow-hidden bg-sky-100 py-36">
      <div class="flex flex-col items-center text-sky-900">
        ...
        <div class="grid items-start grid-flow-row gap-4 mt-4 xl:grid-flow-col xl:auto-cols-auto">
          <ScheduleCard :title="$t('index.section_whatitdoes.cards[0].title')" :description="$t('index.section_whatitdoes.cards[0].description')" :duration="$t('index.section_whatitdoes.cards[0].duration')" dot-class="bg-work" :class="[{ 'shadow-lg shadow-slate-700/30 !bg-slate-600': section2selectedBox === 1 }]" />

          <IconNext class="self-center hidden xl:block" />

          <ScheduleCard :title="$t('index.section_whatitdoes.cards[1].title')" :description="$t('index.section_whatitdoes.cards[1].description')" :duration="$t('index.section_whatitdoes.cards[1].duration')" dot-class="bg-shortpause" :class="[{ 'shadow-lg shadow-slate-700/30 !bg-slate-600': section2selectedBox === 2 }]" />

          <IconNext class="self-center hidden xl:block" />
          <IconDots class="self-center hidden xl:block" />
          <IconNext class="self-center hidden xl:block" />

          <ScheduleCard :title="$t('index.section_whatitdoes.cards[2].title')" :description="$t('index.section_whatitdoes.cards[2].description')" :duration="$t('index.section_whatitdoes.cards[2].duration')" dot-class="bg-longpause" :class="[{ 'shadow-lg shadow-slate-700/30 !bg-slate-600': section2selectedBox === 3 }]" />
        </div>
      </div>
    </Section>

    <!-- Section 3: Features -->
    <Section ref="section-3" class="flex flex-col items-center justify-start px-4 overflow-hidden text-center bg-amber-50 md:justify-center py-36">
      <h2 class="text-5xl font-bold tracking-tight uppercase text-amber-900" v-text="$t('index.section_features.title')" />

      <div class="grid max-w-5xl grid-flow-row grid-cols-2 gap-4 mt-8 text-lg xl:text-xl xl:grid-cols-4 xl:gap-8">
        <div v-for="(feature, index) in section3.smallFeatures" :key="feature" :class="['transition duration-1000 py-4 px-4 rounded-lg text-gray-900 text-opacity-80 flex flex-col justify-center', { '!text-gray-100 text-opacity-100 bg-slate-800': section3.activeFeature === index }]" v-text="$t('index.section_features.list.' + feature)" />
      </div>
    </Section>

    <!-- Section 4: FAQ -->
    <Section ref="section-4" class="flex flex-col items-center justify-center overflow-hidden text-center bg-amber-300 py-36">
      ...
      <div class="mt-8 px-4 w-full xl:w-[1280px] text-left flex flex-col space-y-2">
        <details v-for="(question, i) in faq" :key="'faq-' + i" class="w-full p-4 text-gray-700 transition bg-gray-100 rounded-lg open:bg-gray-50 ring-1 ring-transparent open:ring-gray-400 shadow-slate-400/30 open:shadow-lg open:text-gray-900" :open="openfaq === i" @click.prevent="openfaq = i">
    ...
    <Section ref="section-5" class="flex flex-col items-center justify-start px-4 overflow-hidden text-center md:justify-center bg-stone-100 py-36">
      ...
      <div class="flex flex-row items-center space-x-2 text-sm">
        <a :aria-label="$t('index.alt.links.share.twitter')" href="https://twitter.com/AnotherPomodoro?utm_source=AnotherPomodoro&utm_medium=web&utm_content=home" class="rounded-full w-12 h-12 bg-[#1da1f2] text-white flex flex-row items-center justify-center space-x-1 transition-colors">
          <AboutTwitter size="24" />
        </a>
        <a :aria-label="$t('index.alt.links.share.facebook')" href="http://www.facebook.com/share.php?u=https://another-pomodoro.netlify.app" class="rounded-full w-12 h-12 bg-[#1877f2] text-white flex flex-row items-center justify-center space-x-1 transition-colors">
          <AboutFacebook size="24" class="translate-x-[-1px]" />
        </a>
        <a :aria-label="$t('index.alt.links.share.reddit')" href="https://reddit.com/submit?url=https://another-pomodoro.netlify.app" class="rounded-full w-12 h-12 bg-[#ff4500] text-white flex flex-row items-center justify-center space-x-1 transition-colors">
          <AboutReddit size="24" />
        </a>
      </div>
