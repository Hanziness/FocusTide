...
<template>
  <div role="checkbox" :tabindex="disabled ? -1 : 0" class="relative flex flex-row items-center transition bg-opacity-25 rounded-full cursor-pointer group w-14 h-7 focus:ring ring-theme" :class="[{ 'bg-theme bg-opacity-25': !!value, 'bg-slate-900 bg-opacity-25 dark:bg-slate-200 dark:bg-opacity-25': !value }]" @click="computedValue = !value">
    ...
    <!-- Toggle head -->
    <div class="absolute h-full transition-all duration-300 origin-center scale-75 rounded-full aspect-square" :class="{ 'left-full -translate-x-full bg-theme': !!props.value, 'left-0 bg-slate-800 bg-opacity-80 dark:bg-slate-200 dark:bg-opacity-60': !props.value }" />
  </div>
</template>
